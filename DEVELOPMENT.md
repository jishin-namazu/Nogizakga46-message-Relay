...
- 保存正式消息的推送结果日志。
- 独立轮询官网公开 `/s/n46/api/list/blog` JSONP 接口，只保存 BLOG 去重/推送元数据，不保存正文或评论。
...
- 新增 BLOG Tab：直接从乃木坂46官网分页同步全部 BLOG 和成员目录，按官网分类显示成员头像网格多选模块，并通过列表右侧的筛选图标打开成员选择；时间最新/最早排序使用带滑动指示器的 Tab Switcher，默认最新优先。列表卡片按成员信息、粗体标题、已保存的标题译文和无裁切圆角图片排列，列表和详情间使用与消息成员页相同的过渡动画。成员目录每次同步都从 `/s/n46/api/list/member` 替换本地快照，因此官网成员变动会自动反映到筛选界面。
- BLOG 列表每页 20 篇，支持上一页、下一页和输入页码跳转，筛选或排序变更后回到第一页。
- BLOG 正文和图片保存/缓存在客户端侧，官网 API 不含评论字段，客户端也不请求或解析评论。
- 只有 FCM 首次插入的新 BLOG 标记未读，历史/前台同步不制造未读；BLOG Tab 显示总未读徽标，列表卡片显示未读标记，打开详情即标记该篇已读。
...
可从 `server/` 运行 `npm run audit:blogs` 实时审计官网全部分页。脚本会校验首尾 `count` 稳定、返回行数、唯一 ID、必需字段、时间顺序，并将全局列表中的成员计数与每个 `ct` 过滤的官网计数逐一对比。如果审计期间发布了新 BLOG，脚本会失败并要求重跑，不会把不稳定快照误报为完整。
...
客户端本地 SQLite schema v7 保留消息的 `is_unread`，并包含 `blog_posts`、BLOG `is_unread`、`blog_members` 和 `sync_state`。`blog_members` 保存官网成员目录的最新快照。升级时已有消息/BLOG 以 `0`（已读）作为基线；v7 会清除旧 BLOG 译文的完成标记，使其在下次打开时按逐行换行保护规则重新生成。
...
服务端表只保存 BLOG ID、成员、标题、时间、封面和链接等推送去重元数据，不保存正文。Android 的同名本地表保存从官网直接取得的正文 HTML、未读状态和按需生成的本地译文。

### `blog_sync_state`

保存服务端 BLOG monitor 最近一次完整成功轮询的头部 ID。增量轮询必须追溯到这个精确 ID 才会移动边界和发送待处理推送；中途请求/数据库失败时边界保持不变，下一轮不会被上次已写入的部分新 ID 误导而提前停止。
...
- 首层按成员分组。
- “最近收到”显示最近六个成员；有未读消息时，头像右上角显示红点。
- “全部成员”显示成员会话和最后一条预览，不显示消息总数；仅在存在未读时于右侧显示红色未读徽标，超过 99 条显示 `99+`。
- 底部“消息”Tab 显示全局未读总数，超过 99 条显示 `99+`。
- 只有 FCM 首次插入的消息会标记为未读；重复推送不会改写状态，历史同步和旧版本迁移数据默认已读。
- 进入成员会话（包括点击通知进入）会把该成员当前未读消息一次性标记为已读。App 位于前台且正在查看该成员时，新收到的消息直接按已读保存。
- 进入成员后，每页 20 条。
- 点击中间页码按钮可输入页码跳转。
...
- BLOG 翻译仅在打开详情时按需入队，避免首次回填数千篇历史 BLOG 时批量消耗模型配额；入队后使用应用级后台队列继续执行，离开详情或切换 Tab 不会取消。BLOG 详情的“重新翻译”同样会清空旧译文并使用当前模型重译整篇。
- BLOG 整篇只提交一次模型请求；连续三个及以上换行定义翻译插入的段落边界。段落内部按原文文本行生成模型片段，返回后在本地重建，因此每个 BLOG 译文块的换行位置和数量与原文块一致。BLOG 翻译过程不在详情页显示状态文字，重新翻译按钮位于成员信息行右侧。
